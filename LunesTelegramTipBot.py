...
BOTNAME  = 'Gaspar Lunes Token @GasparLunesTokenbot'
TOKEN    = '613922783:AAEhuCg9ssje8n6ngX07EYVndbc2XvA1QEQ'
LOG_FILE = '____CHANGEME____'
DB_FILE  = '____CHANGEME____'
APP_EXE  = '____CHANGEME____'
...
def send_message(bot, cid, msg, mode):
    if "commands" not in msg and "Coinlib" not in msg:
        logger.info("{}: {}".format(BOTNAME, msg))
...
def info(bot, update):
    ...
    use them like so:
     ...
     WHERE:
    ` 
       [address] = withdraw #Lunes address
          [user] = telegram username 
        [amount] = amount of #Lunes to utilise 
    `

     *NOTE*:
      - don't deposit a significant amount of #Lunes through this #BOT
      - make sure that you enter a valid #Lunes address when you perform a withdraw
      - we are not responsible of your funds if something bad happen to this #BOT 
     ```
          USE THIS #BOT AT YOUR OWN RISK
     ```
...
def tip(bot, update):
    ...
    if amount > 100000:
        msg = "Please send a lower amount (max: 100,000 Lunes)!"
        send_message(bot, chat_uuid, msg, False)
        return False
    if target == user_name:
        msg = "You can't tip yourself silly.!"
    ...
    msg = '@{0} tipped @{1} of {2} Lunes'.format(user_name, target, amount)
    send_message(bot, chat_uuid, msg, False)
...
def balance(bot, update):
    ...
    msg = '@{0} your current balance is: {1} Lunes'.format(user_name, balance)
    send_message(bot, chat_uuid, msg, False)
...
def withdraw(bot, update):
    ...
    if len(message) != 3:
        ...
        return False
    if not isValidAddress(message[1]):
        msg = 'Please input a valid Lunes address!'
        send_message(bot, chat_uuid, msg, False)
        return False
    if not isValidAmount(message[2]):
        msg = 'Please input a valid amount (ex: 1000)!'
    ...
    msg = '@{0} has successfully withdrew {2} Lunes to address: {1}'\
              .format(user_name, address, amount)
    send_message(bot, chat_uuid, msg, False)
...
def price(bot, update):
    ...
    getmarket = 'https://coinlib.io/coin/LNS2/Lunes'
    response  = requests.get(getmarket)
    json_data = response.json()
    
    if not json_data:
        msg = 'Failed to get data from Coinlib exchange!'
        send_message(bot, chat_uuid, msg, False)
        return False
